...
internal object A11yScrollBoundEdgeHaptics {
    ...
    // Evicts the least recently used entry when size > 128
    private val perSurface = object : LinkedHashMap<String, ScrollEdgeState>(
        16, 0.75f, true
    ) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, ScrollEdgeState>) =
    ...
    fun onViewScrolled(event: AccessibilityEvent): Result {
        ...
        return when (action) {
            ScrollEdgeAction.REACHED_TOP,
            ScrollEdgeAction.REACHED_BOTTOM -> Result.PlayEdgeHaptic
            null -> Result.NoHaptic
        }
    }

    private fun keyFor(e: AccessibilityEvent): String? {
        val source: AccessibilityNodeInfo? = e.source
        return if (source != null) {
            ...
            e.className?.toString()?.let { cn -> "w${e.windowId}\u001f$cn" }
        }
    }
...
internal data class ScrollEdgeState(val lastScrollY: Int? = null)
internal data class ScrollEdgeSnapshot(val scrollY: Int, val maxScrollY: Int)
...
internal fun advance(
    ...
    if (my <= 0) return ScrollEdgeState(lastScrollY = snap.scrollY.coerceAtLeast(0)) to null
